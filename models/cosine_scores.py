...
class RAGModel:
    ...
    def batch_generate_answer(self, batch: Dict[str, Any]) -> List[str]:
        ...
        for _idx, interaction_id in enumerate(batch_interaction_ids):
            ...
            ## bias term, maybe some embeddings are worth more than others
            bias_c = 0.9
            bias_q = 1 - bias_c

            cosine_scores = ( (bias_c*relevant_chunks_embeddings) * (bias_q*query_embedding)).sum(1)
        ...
        # don't pass in cosine_scores if u don't want to explicitly give the model the scores via prompt
        formatted_prompts = self.format_prompts(queries, query_times, batch_retrieval_results, cosine_scores)
    ...
    def format_prompts(self, queries, query_times, batch_retrieval_results=[], cosine_scores = False):
        """
        ...
        for _idx, query in enumerate(queries):
            ...
            ## added explicit cosine similarity scores to prompt
            if cosine_scores:
                user_message += f"Use the provided cosine similarity scores to help you answer the question. Cosine similarity Scores: {cosine_scores}\n"
